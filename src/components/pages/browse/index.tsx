...
export default () => {
    ...
    const initQuery: any = () => {
        ...
        const ret = {
            query: hasQueryInQueryString? hasQueryInQueryString[1]:'',
    ...
    const updateQueryString = () => {
        const uri = `/#/browse?query=${query}&pageNumber=${pageNumber}&authorIds=${authorIds}&channelIds=${channelIds}`;
        ...
        {quotes.content.map(({video, author, quote, starttime}: Quote, idx:number) => (
            ...
            <div 
                ...
                <CardFeatured
                    title={author.name}
                    subtitle={
                        `${(video && video.channel)? `${video.channel.name}`:
